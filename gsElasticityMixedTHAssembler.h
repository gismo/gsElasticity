...
/** @brief Assembles linear and non linear (near) incompressible elasticity matrices with mixed TH elements for 2D plain strain and 3D continua.
...
*/
template <class T>
class gsElasticityMixedTHAssembler : public gsAssemblerBase2<T>
{
public:
...
/** @brief Constructor of the assembler object.
    ...
    \ingroup Assembler
*/
    gsElasticityMixedTHAssembler(  gsMultiPatch<T> const & patches,
...
public:
    ...
    void assemble(const gsMultiPatch<T> & deformed, 
		          const gsMultiPatch<T> & deformed_p);
...
#include GISMO_HPP_HEADER(gsElasticityMixedTHAssembler.hpp)
#endif
