...
int main(int argc, char *argv[])
{
    ...
    real_t solverTime = 0.0;
    gsInfo<<std::setw(20)<<"Iteration"<<std::setw(20)<<"||dU||/||U||"<<std::setw(20)<<"el. assembly"<<std::setw(20)<<"el. solver"<<std::setw(20)<<"\n";
    for (index_t it=0; it!=maxIt; ++it)
    {
        ...
        gsInfo<<std::setw(20)<<it<<std::setw(20)<<du.norm()/u.norm()<<std::setw(20)<<assemblyTime<<std::setw(20)<<solverTime<<std::setw(20)<<"\n";
    ...
    for (size_t p=0; p!=mp.nPatches(); ++p)
        mp_def.patch(p).coefs() += displacement.patch(p).coefs();

    // Initialize the function for the elastic energy
    gsMaterialEval<real_t,gsMaterialOutput::Psi> Psi(&material,mp,mp_def);

    gsField<> solutionField(mp,displacement);
    if (true)
    {
        gsWriteParaview(mp,Psi,"Psi",10000);
        gsWriteParaview(solutionField,"solutionField",10000,false);
    }
    ...
    gsMatrix<> deltaD;
    gsInfo<<std::setw(20)<<"Iteration"<<std::setw(20)<<"||dD||/||D||"<<std::setw(20)<<"\n";
    for (index_t it=0; it!=maxIt; ++it)
    {
        // // Using assembled Residual
        // gsMultiPatch<> Ddot;
        // pfAssembler.assembleResidual(damage,Ddot);
        // pfAssembler.rhs_into(R); // this does NOT contain Qphi*C!!
        // R -= qpsi;
        ...
        gsDebug<<"x^T (M x + q) = "<<deltaD.transpose()*(Q*deltaD + R)<<"\n";
        gsDebug<<"M x + b       = "<<(Q*deltaD + R).transpose().minCoeff()<<"\n";
        gsDebug<<"x             =  "<<deltaD.transpose().minCoeff()<<"\n";

        D += deltaD;

        gsDebug<<"deltaD = "<<deltaD.transpose()<<"\n";
        gsDebug<<"D = "<<D.transpose()<<"\n";

        // Update damage spline
        pfAssembler.constructSolution(D,damage);

        gsInfo<<std::setw(20)<<it<<std::setw(20)<<deltaD.norm()/D.norm()<<std::setw(20)<<"\n";
    ...
    gsWriteParaview(mp,damage,"damage",10000);
