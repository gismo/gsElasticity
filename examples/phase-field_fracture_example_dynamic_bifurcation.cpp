...
int main(int argc, char *argv[])
{
    ...
    if (dimension == 2)
    {
        ...
        index_t ny = 320;  // elements in y direction
    ...
    materialParameters.addReal("E", "Young's modulus", 32e3);
    // Poisson's ratio [-]
    materialParameters.addReal("nu", "Poisson's ratio", 0.);
    // Toughness [kN/mm]
    materialParameters.addReal("Gc", "Toughness", 3e-3);
    // Internal length [mm]
    ...
    std::string inputDir;
    inputDir = "/Users/lucasventavinuela/gismo_fracture/optional/gsElasticity/filedata/phase-field-fracture/AT-2_Order2/bifurcation_TB_1l0_0.25l0/";
    gsFileData<> fd_damage(inputDir + "damage.xml");
    gsMultiPatch<> damage;
    fd_damage.getFirst(damage);
    gsInfo<<"damage size: "<<damage.patch(0).coefs().size()<<"\n";
           ...
           std::string & outputdir)
{
    ...
    gsInfo<<"Plotting MESH"<<step<<"\n";
    std::string filename;
    filename = "mesh_intial";
