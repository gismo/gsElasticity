           ...
           std::string & outputdir)
{
    ...
    // Damping parameters (mass-proportional, damage-dependent)
    // C(d) = c_damp * M_deg(d),  c_damp = 2 * xi * w_m
    // M_deg = rho * int (1-d)^2 N^T N dOmega
    //
    // xi  : damping ratio [-]. Typical range 0.01–0.10 for structural problems.
    //       Default 0.05 (5%). Override via <Real name="xi"> in the XML.
    //
    // w_m : characteristic frequency [rad/s].
    //       Default = 2/tstep (critical Nyquist-like frequency of the time integrator).
    //       This is a standard choice for mass-proportional Rayleigh damping in
    ...
    // Damping matrix C(d) = c_damp * M_deg(d); assembled/updated when damage changes.
    gsSparseMatrix<T> C_damp;
    ...
    while (tcurr<=tend)
    {
        ...
        while(true) // refinement iteration loop
        {
            ...
            // Assemble damage-degraded mass for the damping matrix C(d) = c_damp * M_deg(d).
            // The damage field is already set via material.setParameter(2, damage) above.
            // This is reassembled per refinement iteration; it is also updated inside the
            // staggered loop (after each phase-field step) when c_damp > 0.
            if (c_damp != 0.0)
            {
                elAssembler.assembleDampingMass();
                elAssembler.matrix_into(C_damp);
                C_damp *= c_damp;
            }
            else
                C_damp = gsSparseMatrix<T>(M.rows(), M.cols()); // zero matrix
            ...
            // Predictor step — Zero Acceleration Predictor (ZAP), standard for implicit Newmark.
            // See Hughes IGA book
            // ZAP is preferred over CVP (constant velocity) and CDP (constant displacement)
            // because the velocity predictor already accounts for the acceleration contribution,
            // which directly reduces the initial residual of the C*v damping term and requires
            // fewer Newton/staggered iterations to converge.
            uddot_new.setZero(elAssembler.numDofs(),1); // a_pred = 0
            udot_new  = udot_old + dt * (1.0 - gamma) * uddot_old;
            u_new     = u_old   + dt * udot_old + 0.5 * dt * dt * (1 - 2*beta) * uddot_old;
            ...
            while(true) // Staggered scheme
            {
                ...
                // R = M*a + C(d)*v + K*u - F_ext
                R = M * uddot_new + C_damp * udot_new + K * u_new - Fext;
                Rnorm = R.norm();
                if (stagIt == 0)
                    R0 = (Rnorm > 0) ? Rnorm : 1.0; // For exit criterion (eq. (18))

                // K_eff = K + gamma/(beta*dt)*C(d) + 1/(beta*dt^2)*M
                K += (gamma / (beta * dt)) * C_damp + (1.0 / (beta * math::pow(dt,2))) * M;
                ...
                elAssembler.matrix_into(K);
                elAssembler.rhs_into(Fext);
                // Recheck residual with current C(d), v, a (kinematics already updated above)
                R = M * uddot_new + C_damp * udot_new + K * u_new - Fext;
                Rnorm = R.norm();
                ...
                // Reassemble C(d) with updated damage for the next staggered elasticity step
                if (c_damp != 0.0)
                {
                    material.setParameter(2, damage);
                    elAssembler.initialize();
