...
int main(int argc, char* argv[])
{
    ...
    for (index_t i = 0; i < index_t(timeSpan/timeStep); ++i)
    {
        ...
        gsMultiPatch<> velocityBeam;
        elAssembler.constructSolution(elTimeSolver.velocityVector(),velocityBeam);
        elAssembler.constructSolution(elTimeSolver.displacementVector(),displacement);

        // ALE
        aleAssembler.setFixedDofs(0,boundary::south,velocityBeam.patch(0).boundary(boundary::north)->coefs()*timeStep);
        aleAssembler.setFixedDofs(1,boundary::north,velocityBeam.patch(0).boundary(boundary::south)->coefs()*timeStep);
        aleAssembler.setFixedDofs(2,boundary::west,velocityBeam.patch(0).boundary(boundary::east)->coefs()*timeStep);
        aleNewton.reset();
        aleNewton.solve();
        // velocity ALE
        ...
        // FLOW
        nsAssembler.setFixedDofs(3,boundary::south,velocityBeam.patch(0).boundary(boundary::north)->coefs());
        nsAssembler.setFixedDofs(4,boundary::north,velocityBeam.patch(0).boundary(boundary::south)->coefs());
        nsAssembler.setFixedDofs(5,boundary::west,velocityBeam.patch(0).boundary(boundary::east)->coefs());
