...
int main(int argc, char* argv[])
{
    ...
    gsMultiPatch<> geoALEBase; // this is a part of the flow geometry; we deform only this part to save memory and time
    gsReadFile<>(filenameALE, geoALEBase);
    ...
    gsElasticityAssembler<real_t> aleAssemblerBase(geoALEBase,basisALE,bcInfoALE,gALE);
    aleAssemblerBase.options().setReal("PoissonsRatio",meshPR);
    aleAssemblerBase.options().setInt("MaterialLaw",material_law::neo_hooke_ln);
    //aleAssembler.options().setSwitch("LocalStiff",true);
    gsInfo << "Initialized elasticity system for ALE with " << aleAssemblerBase.numDofs() << " dofs.\n";
    ...
    std::ofstream file;
    file.open("flappingBeam_mesh_lin.txt");
    file << 0. << " " << 0. << std::endl;
    ...
    aleAssemblerBase.constructSolution(gsMatrix<>::Zero(aleAssemblerBase.numDofs(),1),ALE);
    if (numPlotPoints > 0)
    {
        ...
        plotDeformation(geoALEBase,ALE,"flappingBeam_mesh_mesh",collectionMesh,0);
    }
    ...
    for (index_t i = 0; i < index_t(timeSpan/timeStep); ++i)
    {
        ...
        index_t bad = aleAssemblerBase.checkSolution(ALE);
        if (bad != -1 && stop)
        {
        ...
        if (numPlotPoints > 0)
        {
            ...
            plotDeformation(geoALEBase,ALE,"flappingBeam_mesh_mesh",collectionMesh,i+1);
        }
