...
int main(int argc, char *argv[])
{
    ...
    // real_t E = 210e3; // N/mm^2
    real_t E = 210;   // kN/mm^2
    // Poisson's ratio [-]
    real_t nu= 0.3;
    // Toughness [N/mm]
    // real_t Gc= 2.7; // N/mm
    real_t Gc= 2.7e-3; // kN/mm
    ...
    // Shear:
    // bc_u.addCondition(boundary::north,condition_type::dirichlet,0,1);
    // bc_u.addCondition(boundary::north,condition_type::dirichlet,&u_y,0);
    // Tension:
    ...
    for (index_t it=0; it!=maxIt; ++it)
    {
        ...
        std::string filename;
        filename = "Psi_"+util::to_string(it);
        gsWriteParaview(mp,Psi,filename,10000);
        filename += "0";
        psiCollection.addPart(filename,it,"Solution",0);

        filename = "displacement_"+util::to_string(it);
        gsWriteParaview(mp,displacement,filename,10000);
        filename += "0";
        displCollection.addPart(filename,it,"Solution",0);
        ...
        filename = "damage_" + util::to_string(it);
        gsWriteParaview(mp,damage,filename,10000);
        filename += "0";
        damageCollection.addPart(filename,it,"Solution",0);
