...
#   else
    ...
    collection_disp.options().setInt("numPoints", 3000); 

    gsParaviewCollection collection_dmg("ParaviewOutput_fracture/dmg", &ev_d);
    collection_dmg.options().setSwitch("plotElements", true);
    ...
    for (index_t step = 0; step!=maxSteps; step++)
    {
        ...
        for (index_t stag = 0; stag!=maxStag; stag++)
        {
            ...
            for (index_t it_eq = 0; it_eq!=maxIt; it_eq++) 
            {
                ...
                {
                    gsInfo<<"\tEquilibrium converged in "<<it_eq+1<<" iterations\n\n";
            ...
            {                
                    ...
                    {
                        gsInfo<<"\tPhase-field converged in "<<it_pf+1<<" iterations\n\n";
                        converged = true;
                        break;
                    }
            ...
            // Staggered check! Recompute residual (with updated solution)
            A_u.clearMatrix();
            A_u.clearRhs();
        ...
        {
            // Export the mesh
            collection_disp.newTimeStep(&mp);
            collection_dmg.newTimeStep(&mp);
            collection_disp.addField(unew,"displacement");
            collection_dmg.addField(dnew,"damage");
            collection_disp.saveTimeStep();
            collection_dmg.saveTimeStep();
        }
