    ...
    int max_it = 50)
{
    ...
    // Assemble the iterate w to get phi'(lambda_0) 
    assembler.initialize();
    assembler.assemble(w);
    ...
    gsMatrix<T> w_right = w + initial_lambda * dw; //lambda is initially 1
    assembler.assemble(w_right);
    assembler.matrix_into(mat);
    assembler.rhs_into(rhs);
    ...
    while (it < max_it)
    {
        if (math::abs(fty) / ynorm<= atol)
            break;
        if (math::abs(fty / fty_initial) <= rtol)
            break;
        if (math::abs(lambda - lambda_old) < ltol)
            break;
        ...
        gsMatrix<T> w_new = w + lambda * dw;
        assembler.assemble(w_new);
        assembler.matrix_into(mat);
        assembler.rhs_into(rhs);
